...
void ChannelMixingLayer::initialize() {
  ...
  // Try filling with a simpler approach - assign to all at once
  printf("[CM.init] Attempting to fill shift_ with fill function\n");
  fflush(stdout);
  
  std::fill(shift_.begin(), shift_.end(), 0.5f);
  
  printf("[CM.init] shift_ filled successfully with std::fill\n");
  fflush(stdout);
