...
extern "C" {
    ...
    // Test function that does basic quantization operations without std::vector
    __declspec(dllexport) int test_basic_quantize_ops() {
        try {
            std::cout << "DEBUG: test_basic_quantize_ops called" << std::endl;
            ...
            std::cout << "DEBUG: basic quantization completed successfully" << std::endl;
            return 1; // Success
        } catch (const std::exception& e) {
            std::cout << "DEBUG: Exception in basic quantize ops: " << e.what() << std::endl;
            return -1; // Failure
        } catch (...) {
            std::cout << "DEBUG: Unknown exception in basic quantize ops" << std::endl;
            return -2; // Failure
        }
    }
