...
struct SSMParameters {
    ...
    size_t memory_bytes() const {
        return (W_x.size() + W_z.size() + W_B.size() + W_C.size() + 
                ...
                conv_weight.size() + conv_bias.size() + W_out.size()) * sizeof(float);
    }
};
...
struct SSMStats {
    ...
    double get_avg_time_per_token_ms() const {
        return total_tokens_processed > 0 ? 
               total_time_ms / total_tokens_processed : 0.0;
    }
    
    double get_throughput_tokens_per_sec() const {
        return total_time_ms > 0 ? 
               (total_tokens_processed * 1000.0) / total_time_ms : 0.0;
    }
