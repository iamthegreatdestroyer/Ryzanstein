...
void ChannelMixingLayer::matrix_multiply_(
  ...
  printf("[CM.matrix_mult] Output zeroed\n");
  fflush(stdout);
  
  // Completely unroll the dot product - no loops at all!
  // For 64-element vectors, manually add all 64 multiplications
  if (m > 0 && n >= 64) {
    float result = 0.0f;
    
    // Manually add all 64 multiplications
    result += A[0] * x[0];
    result += A[1] * x[1];
    result += A[2] * x[2];
    result += A[3] * x[3];
    ...
    printf("[CM.matrix_mult] Row 0 computed\n");
    fflush(stdout);
  }
