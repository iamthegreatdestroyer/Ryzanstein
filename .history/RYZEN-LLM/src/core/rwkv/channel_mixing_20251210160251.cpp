...
void ChannelMixingLayer::matrix_multiply_(
  ...
  // CRITICAL: Any loop causes crashes. Use fully unrolled computation instead.
  // Manually compute: y[0] = A[0]*x[0] + A[1]*x[1] + ... + A[n-1]*x[n-1] + bias[0]
  
  float result = 0.0f;
  
  // Unroll computation for all n elements
  // For n=64, this expands to 64 multiplication-additions
