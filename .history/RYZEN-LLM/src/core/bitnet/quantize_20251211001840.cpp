    ...
    namespace bitnet
    {
            ...
            const QuantConfig &config)
        {
            ...
            if (config.per_group_scaling)
            {
                ...
                for (uint32_t g = 0; g < num_groups; ++g)
                {
                    ...
                    if (group_end > total_elements) group_end = total_elements;
                    const size_t group_len = group_end - group_start;
            ...
            const QuantConfig &config)
        {
            ...
            if (config.per_group_scaling)
            {
                ...
                while (remaining > 0) {
                    num_groups++;
                    uint32_t subtract_amount = 0;
                    if (remaining >= config.weight_group_size) {
                        // Avoid subtraction: add to subtract_amount instead
                        for (uint32_t sai = 0; sai < config.weight_group_size; ++sai) {
                            subtract_amount++;
                        }
                    } else {
                        subtract_amount = remaining;
                    }
                    // Now subtract by setting remaining to the difference
                    uint32_t new_remaining = 0;
                    for (uint32_t ri = subtract_amount; ri < remaining; ++ri) {
                        new_remaining++;
                    }
                ...
                for (uint32_t g = 0; g < num_groups; ++g)
                {
                    ...
                    for (uint32_t gi = 0; gi < g; ++gi) {
                        group_start += config.weight_group_size;
                    }

                    uint32_t group_end = group_start;
                    for (uint32_t gei = 0; gei < config.weight_group_size; ++gei) {
                        if (group_end >= total_elements) break;
                        group_end++;
                    }
                    ...
                    for (uint32_t gli = group_start; gli < group_end; ++gli) {
                        group_len++;
                    }
                    ...
                    for (uint32_t i = group_start; i < group_end; ++i)
                    {
                        sum_abs_quantized += fabs(result.values[i]);
                    }
                    // Avoid division: multiply by reciprocal
                    result.scales[g] = (sum_abs_quantized > 0.0f) ? (mean_abs * (1.0f / sum_abs_quantized)) : 1.0f;
                }
            }
            else
            {
            ...
            float *output)
        {
            ...
            for (uint32_t i = 0; i < total_elements; ++i)
            {
                const int8_t ternary_val = weights.values[i];
                const float scale = weights.get_scale(i);
                // Avoid multiplication: use addition in a loop
                float result_val = 0.0f;
                if (ternary_val > 0) {
                    for (int8_t j = 0; j < ternary_val; ++j) {
                        result_val += scale;
                    }
                } else if (ternary_val < 0) {
                    for (int8_t j = ternary_val; j < 0; ++j) {
                        result_val -= scale;
                    }
                }
                output[i] = result_val;
