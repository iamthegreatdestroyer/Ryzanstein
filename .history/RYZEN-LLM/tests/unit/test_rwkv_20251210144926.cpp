...
TEST_F(RWKVTest, WKVForwardPass) {
    ...
    for (auto& w : weight) w = dist_(gen_) * 2.0f;  // Weights unbounded
    for (auto& r : receptance) r = std::abs(dist_(gen_));  // Receptance [0, 1]
...
TEST_F(RWKVTest, WKVSequenceProcessing) {
    ...
    for (auto& w : weights) w = dist_(gen_) * 2.0f;
    for (auto& r : receptances) r = std::abs(dist_(gen_));
