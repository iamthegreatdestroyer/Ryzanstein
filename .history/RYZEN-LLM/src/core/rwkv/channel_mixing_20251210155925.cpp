...
void ChannelMixingLayer::matrix_multiply_(
  ...
  printf("[CM.matrix_mult] matrix_mult start m=%u n=%u\n", m, n);
  fflush(stdout);
  
  // Do NOT use any loops that write to vectors!
  // Instead, use std::inner_product or manual unrolled computation
  
  // Create a simple approach: process 1 row at a time using std::accumulate-like logic
  for (uint32_t row = 0; row < m; ++row) {
    float sum = 0.0f;
    uint32_t row_start = row * n;
    
    // Compute dot product: A[row,:] . x
    // Use manual summation to avoid any loop vector access issues
    sum = A[row_start + 0] * x[0];
    if (n > 1) sum += A[row_start + 1] * x[1];
    if (n > 2) sum += A[row_start + 2] * x[2];
    if (n > 3) sum += A[row_start + 3] * x[3];
    if (n > 4) sum += A[row_start + 4] * x[4];
    
    // Handle remaining elements one at a time via goto-style unroll
    for (uint32_t j = 5; j < n; ++j) {
      sum = sum + A[row_start + j] * x[j];
    }
    
    y[row] = sum;
    if (bias && !bias->empty() && row < bias->size()) {
      y[row] += (*bias)[row];
    }
  }
  
  printf("[CM.matrix_mult] Done\n");
  fflush(stdout);
}
