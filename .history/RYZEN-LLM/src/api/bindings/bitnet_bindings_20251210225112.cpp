...
extern "C" {
    ...
    // Test function that does quantization without std::vector
    __declspec(dllexport) int test_quantize_no_vector() {
        try {
            std::cout << "DEBUG: test_quantize_no_vector called" << std::endl;
            
            // Use raw arrays instead of std::vector
            const int size = 16;
            float weights[size] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f,
                                  9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f};
            int8_t quantized[size];
            float scales[1];
            ...
            for (int i = 0; i < size; ++i) {
                ...
                std::cout << "DEBUG: weight " << i << ": " << w << " -> " << (int)quantized[i] << std::endl;
            }
            
            // Compute scale
            scales[0] = (sum_abs_quantized > 0) ? (sum_abs_original / sum_abs_quantized) : 1.0f;
            std::cout << "DEBUG: scale computed: " << scales[0] << std::endl;
            
            std::cout << "DEBUG: quantization without std::vector completed successfully" << std::endl;
            return 1; // Success
        } catch (const std::exception& e) {
            std::cout << "DEBUG: Exception in quantize_no_vector: " << e.what() << std::endl;
            return -1; // Failure
        } catch (...) {
            std::cout << "DEBUG: Unknown exception in quantize_no_vector" << std::endl;
            return -2; // Failure
        }
    }
