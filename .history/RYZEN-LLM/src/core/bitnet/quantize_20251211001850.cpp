    ...
    namespace bitnet
    {
            ...
            const QuantConfig &config)
        {
            ...
            if (config.per_group_scaling)
            {
                ...
                uint32_t temp_elements = total_elements;
                while (temp_elements > 0) {
                    num_groups++;
                    if (temp_elements >= config.weight_group_size) {
                        // Subtract by counting down
                        uint32_t new_temp = 0;
                        for (uint32_t i = config.weight_group_size; i < temp_elements; ++i) {
                            new_temp++;
                        }
                        temp_elements = new_temp;
                    } else {
                        temp_elements = 0;
                    }
                }
