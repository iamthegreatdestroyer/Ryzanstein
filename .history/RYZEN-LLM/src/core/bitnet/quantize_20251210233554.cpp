...
namespace ryzen_llm
{
    namespace bitnet
    {
        ...
        TernaryWeight quantize_weights_ternary_scalar(
            ...
            const QuantConfig &config)
        {
            printf("DEBUG: quantize_weights_ternary_scalar called with rows=%zu, cols=%zu\n", rows, cols);
            printf("DEBUG: per_group_scaling=%d\n", config.per_group_scaling ? 1 : 0);
            ...
            {
                std::cout << "DEBUG: Starting per-layer quantization" << std::endl;
                // Per-layer quantization
                // Compute mean absolute value across entire weight matrix
                std::cout << "DEBUG: Computing mean_abs" << std::endl;
                float mean_abs = 0.0f;
                printf("DEBUG: Starting mean_abs loop with total_size=%zu\n", total_size);
                for (size_t i = 0; i < total_size; ++i)
                {
                    printf("DEBUG: i=%zu, weights[%zu]=%f\n", i, i, weights[i]);
                    // Use fabs instead of std::abs to avoid AVX-512
                    float abs_val = fabs(weights[i]);
                    printf("DEBUG: fabs(weights[%zu])=%f\n", i, abs_val);
                    mean_abs += abs_val;
                    printf("DEBUG: mean_abs after addition=%f\n", mean_abs);
                }
                printf("DEBUG: About to divide mean_abs=136.000000 by total_size=16\n");
                // Use literal division to avoid AVX-512
                mean_abs = mean_abs / 16.0f;
                printf("DEBUG: mean_abs computed\n");

                const float threshold = 0.7f * mean_abs;
                printf("DEBUG: threshold computed: %f\n", threshold);
                ...
                {
                    printf("DEBUG: Loop iteration i=%zu\n", i);
                    const float w = weights[i];
                    printf("DEBUG: w=%f\n", w);
                    sum_abs_original += fabs(w);
                    printf("DEBUG: sum_abs_original=%f\n", sum_abs_original);

                    if (fabs(w) > threshold)
                    {
                        printf("DEBUG: fabs(w)=%f > threshold=%f, setting to %d\n", fabs(w), threshold, (w > 0) ? 1 : -1);
                        result.values[i] = (w > 0) ? 1 : -1;
                        sum_abs_quantized += 1.0f;
                        printf("DEBUG: sum_abs_quantized=%f\n", sum_abs_quantized);
                    }
                    else
                    {
                        printf("DEBUG: fabs(w)=%f <= threshold=%f, setting to 0\n", fabs(w), threshold);
                        result.values[i] = 0;
                    }
                }
                printf("DEBUG: Quantization loop completed\n");

                // Compute single scale factor for the layer
                result.scales[0] = (sum_abs_quantized > 0)
                                       ? (sum_abs_original / sum_abs_quantized)
                                       : 1.0f;
                printf("DEBUG: Scale factor computed: %f\n", result.scales[0]);
            }
