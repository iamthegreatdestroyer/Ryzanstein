...
void ChannelMixingLayer::matrix_multiply_(
  ...
  printf("[CM.matrix_mult] mult start m=%u n=%u x.size=%zu A.size=%zu y.size=%zu\n", 
         m, n, x.size(), A.size(), y.size());
  fflush(stdout);
  
  // Use pointer-based approach instead of loops
  const float* A_ptr = A.data();
  const float* x_ptr = x.data();
  float* y_ptr = y.data();
  const float* bias_ptr = bias ? bias->data() : nullptr;
  
  printf("[CM.matrix_mult] Pointers acquired\n");
  fflush(stdout);
  
  // Manual loop unrolling - compute each row's dot product
  // Process rows 0-63
  if (m >= 1) {
    float sum0 = A_ptr[0] * x_ptr[0];
    for (uint32_t j = 1; j < n; ++j) sum0 += A_ptr[j] * x_ptr[j];
    y_ptr[0] = sum0 + (bias_ptr ? bias_ptr[0] : 0.0f);
  }
  
  if (m >= 2) {
    float sum1 = A_ptr[n] * x_ptr[0];
    for (uint32_t j = 1; j < n; ++j) sum1 += A_ptr[n + j] * x_ptr[j];
    y_ptr[1] = sum1 + (bias_ptr ? bias_ptr[1] : 0.0f);
  }
  
  // For remaining rows > 2, use a loop
  for (uint32_t i = 2; i < m; ++i) {
    float sum_i = A_ptr[i * n] * x_ptr[0];
    for (uint32_t j = 1; j < n; ++j) {
      sum_i += A_ptr[i * n + j] * x_ptr[j];
    }
    y_ptr[i] = sum_i + (bias_ptr ? bias_ptr[i] : 0.0f);
  }
