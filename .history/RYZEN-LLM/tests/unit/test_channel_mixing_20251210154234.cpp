...
void test_channel_mixing_invalid_config() {
  ...
  try {
    ...
    ASSERT_FAIL("Should throw for zero hidden_dim");
  ...
  try {
    ChannelMixingLayer layer(256, 0, config);
    ASSERT_FAIL("Should throw for invalid ff_expansion");
  } catch (const std::invalid_argument& e) {
...
void test_channel_mixing_initialization() {
  ...
  // Verify state string works
  std::string state = layer.get_state_string();
  ASSERT_TRUE(state.find("Initialized: true") != std::string::npos);
...
void test_channel_mixing_double_init() {
  ...
  try {
    layer.initialize();
    ASSERT_FAIL("Should throw on double initialization");
