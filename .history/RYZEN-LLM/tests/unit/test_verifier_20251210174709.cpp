...
{
protected:
    VerifierConfig default_config{
        .vocab_size = 32000,
        .temperature = 1.0f,
        .rejection_threshold = 0.5f,
...
TEST_F(VerifierTest, ConstructorThrowsOnZeroVocabSize)
{
    ...
    config.vocab_size = 0;
    EXPECT_THROW(Verifier(config), std::invalid_argument);
}
...
TEST_F(VerifierTest, ConstructorThrowsOnInvalidRejectionThreshold)
{
    ...
    config.rejection_threshold = 1.1f;
    EXPECT_THROW(Verifier(config), std::invalid_argument);
}
...
{
    VerifierConfig minimal_config{
        .vocab_size = 100,
        .temperature = 1.0f,
        .rejection_threshold = 0.5f,
        .enable_statistics = false};

    EXPECT_NO_THROW(Verifier verifier(minimal_config));
}
...
TEST_F(VerifierEdgeCaseTest, LargeVocabSize)
{
    ...
    EXPECT_NO_THROW(Verifier verifier(large_config));
}
...
TEST_F(VerifierEdgeCaseTest, ExtremeTemperatures)
{
    ...
    EXPECT_NO_THROW(Verifier verifier(low_temp_config));

    // Very high temperature
    VerifierConfig high_temp_config = default_config;
    high_temp_config.temperature = 100.0f;
    EXPECT_NO_THROW(Verifier verifier(high_temp_config));
}
