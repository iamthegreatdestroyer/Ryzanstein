...
void ChannelMixingLayer::matrix_multiply_(
  ...
  printf("[CM.matrix_mult] Entering matrix_multiply_(), m=%u, n=%u\n", m, n);
  fflush(stdout);
  
  printf("[CM.matrix_mult] A.size()=%zu, x.size()=%zu, y.size()=%zu\n", 
         A.size(), x.size(), y.size());
  fflush(stdout);
  
  // Initialize y with zeros
  std::fill(y.begin(), y.end(), 0.0f);
  printf("[CM.matrix_mult] y filled with zeros\n");
  fflush(stdout);
  
  // Perform matrix multiplication: y = A @ x
  // A is m x n matrix stored in row-major order
  ...
  for (uint32_t i = 0; i < m; ++i) {
    ...
    printf("[CM.matrix_mult] About to write y[%u] = %f\n", i, sum);
    fflush(stdout);
    y[i] = sum;
    printf("[CM.matrix_mult] y[%u] written successfully\n", i);
    fflush(stdout);
  }
  
  printf("[CM.matrix_mult] Matrix multiplication completed\n");
  fflush(stdout);
  
  // Add bias if provided
  if (bias && !bias->empty()) {
    printf("[CM.matrix_mult] Adding bias\n");
    fflush(stdout);
    for (uint32_t i = 0; i < m; ++i) {
      printf("[CM.matrix_mult] About to add bias[%u] = %f to y[%u]\n", i, (*bias)[i], i);
      fflush(stdout);
      y[i] += (*bias)[i];
      printf("[CM.matrix_mult] Bias added to y[%u]\n", i);
      fflush(stdout);
    }
    printf("[CM.matrix_mult] All biases added\n");
    fflush(stdout);
  }
  
  printf("[CM.matrix_mult] matrix_multiply_() completed\n");
  fflush(stdout);
}
