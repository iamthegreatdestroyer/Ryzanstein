...
void ChannelMixingLayer::matrix_multiply_(
  ...
  printf("[CM.matrix_mult] Start\n");
  fflush(stdout);
  
  // Simplest possible approach: delegate to standard library
  // Use fill to zero out, then use only elementary operations
  std::fill(y.begin(), y.end(), 0.0f);
  
  printf("[CM.matrix_mult] Zeroed output\n");
  fflush(stdout);
  
  // Access vectors through const methods only
  if (m > 0 && n > 0) {
    printf("[CM.matrix_mult] Computing row 0\n");
    fflush(stdout);
    ...
    printf("[CM.matrix_mult] Row 0 done\n");
    fflush(stdout);
  }
