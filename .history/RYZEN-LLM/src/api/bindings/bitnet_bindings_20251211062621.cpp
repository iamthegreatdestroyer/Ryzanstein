...
PYBIND11_MODULE(ryzen_llm_bindings, m)
{
    ...
    m.def("quantize_weights_ternary", 
          ...
          py::arg("rows"),
          py::arg("cols"),
          py::arg("config") = ryzen_llm::bitnet::QuantConfig(),
    ...
    m.def("quantize_activations_int8",
          [](py::array_t<float> activations, 
             const ryzen_llm::bitnet::QuantConfig &config) -> ryzen_llm::bitnet::QuantizedActivation {
              auto buf = activations.request();
              return ryzen_llm::bitnet::quantize_activations_int8(
                  static_cast<float*>(buf.ptr), buf.size, config);
          },
          py::arg("activations"),
          py::arg("config") = ryzen_llm::bitnet::QuantConfig(),
    ...
    m.def("dequantize_weights",
          [](const ryzen_llm::bitnet::TernaryWeight &weights) -> py::array_t<float> {
    ...
    m.def("dequantize_activations",
          [](const ryzen_llm::bitnet::QuantizedActivation &activations) -> py::array_t<float> {
    ...
    m.def("compute_quantization_error",
          [](py::array_t<float> original, py::array_t<float> quantized) -> float {
