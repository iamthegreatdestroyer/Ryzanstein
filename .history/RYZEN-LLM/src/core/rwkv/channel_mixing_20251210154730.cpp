...
void ChannelMixingLayer::initialize() {
  ...
  printf("[CM] hidden_dim_ = %u\n", hidden_dim_);
  fflush(stdout);
  
  // Try simpler calculation
  double dbl_dim = static_cast<double>(hidden_dim_);
  printf("[CM] Cast to double: %f\n", dbl_dim);
  fflush(stdout);
  
  double inv_double = 1.0 / dbl_dim;
  printf("[CM] 1.0 / dim (double): %f\n", inv_double);
  fflush(stdout);
  
  double sqrt_double = std::sqrt(inv_double);
  printf("[CM] sqrt result: %f\n", sqrt_double);
  fflush(stdout);
  
  float xavier_scale = static_cast<float>(sqrt_double);
  printf("[CM] Xavier scale cast to float: %f\n", xavier_scale);
  fflush(stdout);

  // Seeded RNG based on layer_id for reproducible initialization
  ...
  fflush(stdout);
  shift_.resize(hidden_dim_, 0.0f);
  printf("[CM] Filling shift\n");
  fflush(stdout);
  for (uint32_t i = 0; i < hidden_dim_; ++i) {
    shift_[i] = dist(rng) * 0.1f;  // Scale down shift initialization
  }
