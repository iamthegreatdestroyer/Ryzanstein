...
PYBIND11_MODULE(ryzen_llm_bindings, m)
{
    ...
    m.def("quantize_activations_int8", &quantize_activations_int8,
          py::arg("activations"), py::arg("size"), py::arg("config") = QuantConfig(),
          "Quantize activations to INT8 format");

    m.def("naive_ternary_matmul", &naive_ternary_matmul,
          py::arg("weights"), py::arg("activations"), py::arg("output"),
