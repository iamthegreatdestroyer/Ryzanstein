...
def install_extension(build_dir: Path, cpp_build_dir: Path):
    ...
    # Find the built extension in the python subdirectory
    # On Windows with MinGW, extensions are .dll files, not .pyd
    extension_files = list((cpp_build_dir / "python").glob("*.pyd")) + list((cpp_build_dir / "python").glob("*.dll"))
...
def test_extension():
    ...
    ryzen_llm_dir = build_python_dir / "ryzen_llm"
    if str(build_python_dir) not in sys.path:
        sys.path.insert(0, str(build_python_dir))
    if str(ryzen_llm_dir) not in sys.path:
        sys.path.insert(0, str(ryzen_llm_dir))
    ...
    try:
        ...
        has_test_function = hasattr(test_module, 'test_function')
        ...
        if has_test_function:
            ...
            available = [attr for attr in dir(test_module) if not attr.startswith('_')]
