...
std::string ChannelMixingLayer::get_state_string() const {
  ...
  if (initialized_) {
    ...
    uint32_t total_kb = 0;
    total_kb += shift_.size() * sizeof(float);
    total_kb += key_proj_w_.size() * sizeof(float);
    total_kb += value_proj_w_.size() * sizeof(float);
    if (!key_proj_b_.empty()) {
      total_kb += key_proj_b_.size() * sizeof(float);
      oss << "    key_proj_b_: " << key_proj_b_.size() * sizeof(float) / 1024.0f << " KB\n";
    }
    if (!value_proj_b_.empty()) {
      total_kb += value_proj_b_.size() * sizeof(float);
