...
class TestFeatureValidation:
    ...
    def test_eos_token_handling(self, engine):
        ...
        # Should complete within reasonable time (test assertion vs actual EOS)
        # Note: Mock engine may not implement EOS, so we check it doesn't run forever
...
class TestErrorHandlingAndRecovery:
    ...
    def test_invalid_token_id(self, engine):
        ...
        try:
            ...
            for token in output:
                if token >= 0:  # Some mock implementations may return -1 as error indicator
                    assert token < E2ETestConfig.VOCAB_SIZE
