...
def test_safetensors_loading():
    ...
    try:
        ...
        # Test loading a small SafeTensors file if available
        # For now, just test that the engine can be created
        engine = ryzen_llm.BitNetEngine()
        print("✓ BitNetEngine created successfully")

        # Test with a config
        config = ryzen_llm.BitNetConfig()
        config.model_path = "models/bitnet/BitNet_1.58B/"  # Test path
        print("✓ BitNetConfig created successfully")
