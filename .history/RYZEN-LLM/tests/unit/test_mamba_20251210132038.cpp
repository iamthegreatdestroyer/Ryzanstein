...
protected:
    ...
    SSMParameters generate_test_parameters(
        ...
        for (auto& val : params.W_x) val = dist(gen_);
        for (auto& val : params.W_z) val = dist(gen_);
        for (auto& val : params.W_B) val = dist(gen_);
        for (auto& val : params.W_C) val = dist(gen_);
        for (auto& val : params.W_dt) val = dist(gen_);
        
        // A should be negative for stability (stored in log scale)
        std::uniform_real_distribution<float> a_dist(-5.0f, -1.0f);
        for (auto& val : params.A_log) val = a_dist(gen_);
        
        for (auto& val : params.D) val = dist(gen_);
        for (auto& val : params.conv_weight) val = dist(gen_);
        for (auto& val : params.conv_bias) val = 0.0f;
        for (auto& val : params.W_out) val = dist(gen_);
    ...
    bool check_output_valid(const float* output, size_t size) {
        for (size_t i = 0; i < size; ++i) {
            ...
            if (std::abs(output[i]) > 1000.0f) return false; // Sanity check
        }
        return true;
    }
