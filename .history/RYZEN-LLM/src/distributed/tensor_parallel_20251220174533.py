...
class RowParallelLinear(TensorParallelLayer):
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Output tensor of shape (..., output_size_per_partition)
        """
        # Local computation: input @ weight.T + bias
        output_parallel = F.linear(input_, self.weight, self.bias)

        # For row-parallel, we don't need all-reduce in forward pass
        # The all-reduce happens in the backward pass for gradient synchronization
...
class ColumnParallelLinear(TensorParallelLayer):
    ...
    def __init__(self,
        ...
        # Shard output dimension for column-parallel
        assert output_size % world_size == 0, f"output_size ({output_size}) must be divisible by world_size ({world_size})"
        self.output_size_per_partition = output_size // world_size
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Output tensor of shape (..., output_size_per_partition) or (..., output_size) if gather_output
        """
        # Local computation: input @ weight.T + bias
        output_parallel = F.linear(input_, self.weight, self.bias)

        if self.gather_output:
            # Gather output across ranks
