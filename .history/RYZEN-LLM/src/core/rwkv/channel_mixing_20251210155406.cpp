...
void ChannelMixingLayer::initialize() {
  printf("[CM.init] Starting initialize()\n");
  fflush(stdout);
  ...
  printf("[CM.init] hidden_dim_=%u\n", hidden_dim_);
  fflush(stdout);

  printf("[CM.init] Creating empty vector\n");
  fflush(stdout);
  std::vector<float> test_vec;
  
  printf("[CM.init] Resizing empty vector to %u\n", hidden_dim_);
  fflush(stdout);
  test_vec.resize(hidden_dim_);
  
  printf("[CM.init] test_vec resized successfully, size=%zu\n", test_vec.size());
  fflush(stdout);
  
  printf("[CM.init] Assigning to test_vec[0]\n");
  fflush(stdout);
  test_vec[0] = 1.23f;
  
  printf("[CM.init] Successfully assigned to test_vec[0]\n");
  fflush(stdout);
  
  printf("[CM.init] Now trying shift_\n");
  fflush(stdout);
  
  // Allocate shift parameter (learnable shift for temporal context)
  printf("[CM.init] Resizing shift_ to %u\n", hidden_dim_);
  fflush(stdout);
  
  shift_.resize(hidden_dim_);
  
  printf("[CM.init] shift_ resized successfully, size=%zu\n", shift_.size());
  fflush(stdout);
  
  printf("[CM.init] Trying to access shift_[0]\n");
  fflush(stdout);
  shift_[0] = 0.5f;
  printf("[CM.init] Successfully assigned to shift_[0]\n");
  fflush(stdout);
