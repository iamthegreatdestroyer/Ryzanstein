...
namespace ryzen_llm
{
    namespace bitnet
    {
        ...
        bool BitNetEngine::load_safetensors_weights(const std::string &weights_path)
        {
        ...
        bool BitNetEngine::parse_safetensors_header(const std::string &header_json, const char *data_start, size_t data_size)
        {
            // Very basic JSON parsing for SafeTensors
            // In production, this should use nlohmann/json or similar

            // Expected format: {"tensor_name": {"shape": [dim1, dim2], "dtype": "F32", "data_offsets": [start, end]}, ...}

            // For now, implement a simple parser for the expected BitNet weight structure
            // This is a placeholder - real implementation would parse the JSON properly

            // BitNet b1.58 expected weight names (from HuggingFace model):
            // - embed_tokens.weight
            ...
            // For this implementation, we'll assume the weights are in the expected order
            // and implement a more robust parser later

            // Placeholder: return false to trigger random weight initialization
            // TODO: Implement proper SafeTensors JSON parsing
            return false;
        }

        bool BitNetEngine::initialize_random_weights()
        {
