...
BitNetEngine::BitNetEngine(const ModelConfig& config)
    ...
    , gate_weights_(config.num_layers, TernaryWeight(config.hidden_size, config.intermediate_size))
    , up_weights_(config.num_layers, TernaryWeight(config.hidden_size, config.intermediate_size))
    , down_weights_(config.num_layers, TernaryWeight(config.intermediate_size, config.hidden_size))
...
void BitNetEngine::attention_layer(
    ...
    // Compute Q, K, V projections using ternary matmul
    naive_ternary_matmul(
