...
void ChannelMixingLayer::matrix_multiply_(
  ...
  printf("[CM.matrix_mult] Starting matrix multiplication\n");
  fflush(stdout);
  
  // Use direct memory operations to avoid any loop issues
  // First clear y
  std::fill(y.begin(), y.end(), 0.0f);
  
  printf("[CM.matrix_mult] Cleared output vector\n");
  fflush(stdout);
  
  // Create temporary accumulator arrays to avoid member vector issues
  std::vector<float> result(m, 0.0f);
  printf("[CM.matrix_mult] Created result vector\n");
  fflush(stdout);
  
  // Perform multiplication with minimal vector access
  for (uint32_t i = 0; i < m; ++i) {
    float accumulator = 0.0f;
    uint32_t row_offset = i * n;
    
    for (uint32_t j = 0; j < n; ++j) {
      accumulator += A[row_offset + j] * x[j];
    }
    
    result[i] = accumulator;
  }
  
  printf("[CM.matrix_mult] Computation done, copying to output\n");
  fflush(stdout);
  
  // Copy result to output
  std::copy(result.begin(), result.end(), y.begin());
  
  printf("[CM.matrix_mult] Result copied\n");
  fflush(stdout);
  
  // Add bias if present
  if (bias && !bias->empty()) {
    std::vector<float> temp_bias(*bias);
    for (uint32_t i = 0; i < m && i < temp_bias.size(); ++i) {
      y[i] += temp_bias[i];
    }
  }
