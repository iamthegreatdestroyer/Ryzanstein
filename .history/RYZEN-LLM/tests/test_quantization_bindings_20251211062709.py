...
class TestWeightQuantization:
    ...
    def test_quantize_weights_preserves_sign(self):
        """Test that quantization preserves sign information for significant values."""
        weights = np.array([
            [5.0, -3.0, 1.5, 0.0],
            [-4.0, 2.0, -2.2, 1.0],
        ], dtype=np.float32)
        ...
        # Check sign preservation for significant values
        # Note: Very small values may round to zero in ternary quantization
        for i in range(weights.shape[0]):
            for j in range(weights.shape[1]):
                orig = weights[i, j]
                dequant = dequantized[i, j]
                
                # Sign should be preserved for non-trivial values
                # Ternary quantization ({-1, 0, +1}) maps small values to zero
                if abs(orig) > 0.5:
                    assert np.sign(orig) == np.sign(dequant), \
                        f"Sign mismatch at [{i},{j}]: {orig} -> {dequant}"
        
        print("✓ Sign preservation verified for significant values")
