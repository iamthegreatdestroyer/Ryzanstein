...
void TimeMixingLayer::initialize() {
    ...
    for (auto& w : weight_r_) w = dist(gen);
    for (auto& w : weight_w_) w = dist(gen);
    for (auto& w : weight_k_) w = dist(gen);
    for (auto& w : weight_v_) w = dist(gen);
    for (auto& w : weight_out_) w = dist(gen);
...
bool TimeMixingLayer::time_shift(const float* current, float* output) {
    ...
    compute_receptance(current, buffer_r_);
...
bool TimeMixingLayer::forward(
    ...
    compute_receptance(buffer_shift_, buffer_r_);  // R ∈ [-1, 1]
    compute_weight(buffer_shift_, buffer_w_);      // W > 0
    compute_key(buffer_shift_, buffer_k_);         // K ∈ ℝ
    compute_value(buffer_shift_, buffer_v_);       // V ∈ ℝ
