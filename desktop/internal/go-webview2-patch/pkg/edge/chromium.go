...
func (e *Chromium) Embed(hwnd uintptr) bool {
	...
	allArgs := append([]string{}, e.AdditionalBrowserArgs...)
	if dbgPort := os.Getenv("RYZANSTEIN_DEBUG_PORT"); dbgPort != "" {
		allArgs = append(allArgs, "--remote-debugging-port="+dbgPort)
	}
	browserArgs := strings.Join(allArgs, " ")
