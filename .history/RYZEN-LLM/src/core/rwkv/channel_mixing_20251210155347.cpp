...
void ChannelMixingLayer::initialize() {
  ...
  printf("[CM.init] hidden_dim_=%u, layer_id_=%u, initialized_=%d\n", 
         hidden_dim_, layer_id_, initialized_ ? 1 : 0);
  fflush(stdout);
  ...
  printf("[CM.init] Allocating shift_ with size %u\n", hidden_dim_);
  fflush(stdout);
  
  shift_.resize(hidden_dim_);
  
  printf("[CM.init] shift_ allocated successfully\n");
  printf("[CM.init] shift_.size()=%zu, shift_.capacity()=%zu\n", 
         shift_.size(), shift_.capacity());
  fflush(stdout);
  
  printf("[CM.init] About to fill shift_ values...\n");
  fflush(stdout);
  
  for (uint32_t i = 0; i < hidden_dim_; ++i) {
    if (i % 10 == 0) {
      printf("[CM.init]   About to fill shift_[%u]\n", i);
      fflush(stdout);
    }
    try {
      shift_.at(i) = 0.01f * (i % 10 - 5) * 0.1f;
      if (i % 10 == 0) {
        printf("[CM.init]   Successfully filled shift_[%u]\n", i);
        fflush(stdout);
      }
    } catch (const std::exception& e) {
      printf("[CM.init] ERROR filling shift_[%u]: %s\n", i, e.what());
      fflush(stdout);
      throw;
    }
  }
  printf("[CM.init] shift_ filled successfully\n");
  fflush(stdout);

  // Allocate key projection weights (hidden_dim x hidden_dim)
  printf("[CM.init] Allocating key_proj_w_ with size %u\n", hidden_dim_ * hidden_dim_);
  fflush(stdout);
  
  uint32_t proj_size = hidden_dim_ * hidden_dim_;
  key_proj_w_.resize(proj_size);
  printf("[CM.init] key_proj_w_ allocated successfully\n");
  fflush(stdout);
  
  printf("[CM.init] About to fill key_proj_w_ values...\n");
  fflush(stdout);
  
  for (uint32_t i = 0; i < proj_size; ++i) {
    if (i % 256 == 0) {
      printf("[CM.init]   Filling key_proj_w_[%u]\n", i);
      fflush(stdout);
    }
    key_proj_w_[i] = 0.01f * ((i % 10) - 5) * 0.1f;
  }
  printf("[CM.init] key_proj_w_ filled successfully\n");
  fflush(stdout);

  // Allocate value projection weights (hidden_dim x hidden_dim)
  printf("[CM.init] Allocating value_proj_w_...\n");
  fflush(stdout);
  
  value_proj_w_.resize(proj_size);
  printf("[CM.init] value_proj_w_ allocated successfully\n");
  fflush(stdout);
  
  printf("[CM.init] About to fill value_proj_w_ values...\n");
  fflush(stdout);
  
  for (uint32_t i = 0; i < proj_size; ++i) {
    if (i % 256 == 0) {
      printf("[CM.init]   Filling value_proj_w_[%u]\n", i);
      fflush(stdout);
    }
    value_proj_w_[i] = 0.01f * ((i % 10) - 5) * 0.1f;
  }
  printf("[CM.init] value_proj_w_ filled successfully\n");
  fflush(stdout);

  // Allocate biases if enabled
  printf("[CM.init] config_.use_bias=%d\n", config_.use_bias ? 1 : 0);
  fflush(stdout);
  
  if (config_.use_bias) {
    printf("[CM.init] Allocating bias vectors with size %u...\n", hidden_dim_);
    fflush(stdout);
    
    key_proj_b_.resize(hidden_dim_, 0.0f);
    value_proj_b_.resize(hidden_dim_, 0.0f);
    printf("[CM.init] Bias vectors allocated\n");
    fflush(stdout);
    
    printf("[CM.init] Filling bias values...\n");
    fflush(stdout);
    
    for (uint32_t i = 0; i < hidden_dim_; ++i) {
      if (i % 10 == 0) {
        printf("[CM.init]   Filling bias[%u]\n", i);
        fflush(stdout);
      }
      key_proj_b_[i] = 0.001f;
      value_proj_b_[i] = 0.001f;
    }
    printf("[CM.init] Biases filled successfully\n");
    fflush(stdout);
  }
  ...
  printf("[CM.init] prev_shifted_ allocated (%zu elements)\n", prev_shifted_.size());
  fflush(stdout);
  
  buffer_key_.resize(hidden_dim_, 0.0f);
  printf("[CM.init] buffer_key_ allocated (%zu elements)\n", buffer_key_.size());
  fflush(stdout);
  
  buffer_key_act_.resize(hidden_dim_, 0.0f);
  printf("[CM.init] buffer_key_act_ allocated (%zu elements)\n", buffer_key_act_.size());
  fflush(stdout);
  
  buffer_value_.resize(hidden_dim_, 0.0f);
  printf("[CM.init] buffer_value_ allocated (%zu elements)\n", buffer_value_.size());
  fflush(stdout);
  
  buffer_gate_.resize(hidden_dim_, 0.0f);
  printf("[CM.init] buffer_gate_ allocated (%zu elements)\n", buffer_gate_.size());
  fflush(stdout);

  printf("[CM.init] About to set initialized_ = true\n");
  fflush(stdout);
  
  initialized_ = true;
  
  printf("[CM.init] initialized_ is now: %d\n", initialized_ ? 1 : 0);
