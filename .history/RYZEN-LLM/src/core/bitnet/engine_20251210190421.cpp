        ...
        bool BitNetEngine::load_weights(const std::string &weights_path)
        {
            ...
            if (!file.is_open())
            {
                ...
                for (uint32_t layer = 0; layer < config_.num_layers; ++layer)
                {
                    ...
                    for (auto &w : temp_q) w = dist(gen);
                    for (auto &w : temp_k) w = dist(gen);
                    for (auto &w : temp_v) w = dist(gen);
                    for (auto &w : temp_o) w = dist(gen);
                    ...
                    for (auto &w : temp_gate) w = dist(gen);
                    for (auto &w : temp_up) w = dist(gen);
                    for (auto &w : temp_down) w = dist(gen);
                    ...
                    for (auto &w : attn_norm_weights_[layer]) w = norm_dist(gen);
                    for (auto &w : mlp_norm_weights_[layer]) w = norm_dist(gen);
                }

                // Final norm
                std::normal_distribution<float> norm_dist(1.0f, 0.02f);
                for (auto &w : final_norm_weights_) w = norm_dist(gen);

                // Output projection (keep as float32)
                for (auto &w : lm_head_weights_) w = dist(gen);
