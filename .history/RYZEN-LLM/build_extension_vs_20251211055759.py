...
def verify_tools(env: dict = None):
    ...
    # Check cl.exe
    try:
        result = subprocess.run(['cl.exe'], capture_output=True, text=True, env=env, timeout=5)
        # cl.exe always returns non-zero when run without args, check stderr
        if 'Microsoft' in result.stderr or 'Visual C++' in result.stderr:
            print("  ✓ CL.exe (Visual Studio C++ Compiler)")
        else:
            print("  ✗ CL.exe not responding correctly")
            return False
    ...
    # Check MSBuild
    try:
        result = subprocess.run(['MSBuild.exe', '/version'], capture_output=True, text=True, env=env, timeout=5)
        if 'Microsoft' in result.stdout:
            print("  ✓ MSBuild")
        else:
            print("  ⚠ MSBuild version unclear")
    except Exception as e:
        print(f"  ⚠ MSBuild: {e}")
...
def configure_cmake(cpp_build_dir: Path, env: dict):
    ...
    # Build cmake command
    cmake_cmd = f'cmake -S "{Path("src").absolute()}" -B "." -DCMAKE_BUILD_TYPE=Release -G "Visual Studio 16 2019" -A x64'
