...
void ChannelMixingLayer::initialize() {
  ...
  printf("[CM.init] Filling shift_ values...\n");
  fflush(stdout);
  
  // Try filling just first value manually
  printf("[CM.init] About to fill shift_[0]\n");
  fflush(stdout);
  shift_[0] = 0.01f * (0 % 10 - 5) * 0.1f;
  printf("[CM.init] Filled shift_[0] successfully\n");
  fflush(stdout);
  
  // Try loop
  printf("[CM.init] Starting for loop, hidden_dim_=%u\n", hidden_dim_);
  fflush(stdout);
  for (uint32_t i = 1; i < hidden_dim_; ++i) {
    printf("[CM.init]   About to fill shift_[%u]\n", i);
    fflush(stdout);
    shift_[i] = 0.01f * (i % 10 - 5) * 0.1f;
    printf("[CM.init]   Filled shift_[%u]\n", i);
    fflush(stdout);
  }
  ...
  printf("[CM.init] Allocating key_proj_w_ (%u x %u = %u elements)\n", 
         hidden_dim_, hidden_dim_, hidden_dim_ * hidden_dim_);
  fflush(stdout);
  
  uint32_t proj_size = hidden_dim_ * hidden_dim_;
  key_proj_w_.resize(proj_size);
  printf("[CM.init] key_proj_w_ allocated, filling values...\n");
  fflush(stdout);
  ...
  printf("[CM.init] value_proj_w_ allocated, filling values...\n");
  fflush(stdout);
  ...
  printf("[CM.init] use_bias=%d\n", config_.use_bias ? 1 : 0);
  fflush(stdout);
  
  if (config_.use_bias) {
    printf("[CM.init] Allocating bias vectors...\n");
    fflush(stdout);
