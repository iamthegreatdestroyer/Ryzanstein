...
namespace ryzen_llm
{
    namespace speculative
    {
            ...
            const std::vector<std::vector<float>> &target_logits)
        {
            ...
            result.num_accepted = result.accepted_tokens.size();
            result.acceptance_rate = result.num_accepted > 0
                                         ? static_cast<float>(result.num_accepted) / draft_tokens.size()
                                         : 0.0f;
        ...
        {
            // Validate input
            if (target_logits.size() != config_.vocab_size)
            {
                return -1;
            }
