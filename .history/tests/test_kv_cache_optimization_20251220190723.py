...
class TestDistributedKVCache:
    ...
    def test_update_and_get_kv(self, distributed_cache):
        ...
        seq_pos = 5
        k = torch.randn(batch_size, distributed_cache.head_dim)
        v = torch.randn(batch_size, distributed_cache.head_dim)
        ...
        assert torch.allclose(k_retrieved.squeeze(1), k, atol=1e-6)
        assert torch.allclose(v_retrieved.squeeze(1), v, atol=1e-6)
