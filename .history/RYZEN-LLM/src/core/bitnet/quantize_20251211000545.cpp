    ...
    namespace bitnet
    {
            ...
            const QuantConfig &config)
        {
            ...
            if (config.per_group_scaling)
            {
                ...
                while (remaining > 0) {
                    ...
                    if (remaining >= config.weight_group_size) {
                        remaining -= config.weight_group_size;
                    } else {
                        remaining = 0;
                    }
                }
            ...
            return result;
                    ...
                    result.scales[g] = (sum_abs_quantized > 0.0f) ? (mean_abs / sum_abs_quantized) : 1.0f;
                }
            }
            else
            {
                ...
                // Compute scale factor for the layer
                float sum_abs_quantized = 0.0f;
                for (uint32_t i = 0; i < total_elements; ++i)
                {
                    sum_abs_quantized += fabs(result.values[i]);
                }
                result.scales[0] = (sum_abs_quantized > 0.0f) ? (mean_abs / sum_abs_quantized) : 1.0f;
            }
        ...
        void dequantize_weights_cpu(
            ...
            float *output)
        {
            uint32_t total_elements = weights.rows * weights.cols;
