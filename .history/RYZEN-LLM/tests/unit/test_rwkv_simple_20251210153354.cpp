...
int main() {
    ...
    try {
        ...
        if (!success) throw std::runtime_error("Forward pass returned false");
    ...
    try {
        ...
        if (!success) throw std::runtime_error("Forward sequence returned false");
    ...
    try {
        ...
        for (int i = 0; i < 3; ++i) {
            TimeMixingConfig config;
            layers.emplace_back(128, i, config);
        }
        ...
        for (auto& layer : layers) {
            ...
            if (!success) throw std::runtime_error("Layer forward failed");
            x = output;
        }
    ...
    try {
        ...
        if (!success) throw std::runtime_error("Time shift returned false");
