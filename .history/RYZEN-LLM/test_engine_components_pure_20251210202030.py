...
def quantize_activations_int8(activations, size, config):
    ...
    if config.symmetric_activations:
        clip_value = min(config.activation_clip_value,
                        max(abs(np.min(activations)), abs(np.max(activations))))
    else:
        clip_value = config.activation_clip_value

    # Compute scale: (2^7 - 1) / clip_value for INT8 range [-127, 127]
    result.scale = 127.0 / clip_value
    result.zero_point = 0  # Symmetric quantization

    # Quantize
    for i in range(size):
        clamped = max(-clip_value, min(clip_value, activations[i]))
        quantized = clamped * result.scale
