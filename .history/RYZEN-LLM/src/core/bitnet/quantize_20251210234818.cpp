...
namespace ryzen_llm
{
    namespace bitnet
    {
        ...
        TernaryWeight quantize_weights_ternary_scalar(
            ...
            const QuantConfig &config)
        {
            // Compute total_size using loop to avoid AVX-512 multiplication
            size_t total_size = 0;
            ...
            else
            {
                ...
                // Use division with total_size to compute mean
                mean_abs = mean_abs / static_cast<float>(total_size);
                ...
                // Use total_size instead of literal 16
                for (size_t i = 0; i < total_size; ++i)
                {
                    const float w = weights[i];
                    sum_abs_original += fabs(w);
