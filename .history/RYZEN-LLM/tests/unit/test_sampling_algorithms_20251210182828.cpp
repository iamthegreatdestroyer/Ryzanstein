...
namespace sampling_algorithms_test
{
    ...
    TEST_F(TopPTest, TopPAccumulatesCorrectly)
    {
        ...
        // Verify cumulative sum is close to p
        float cumsum = 0.0f;
        for (float p : filtered)
        {
            cumsum += p;
        ...
        EXPECT_LE(cumsum, 1.0f); // Could be less due to rounding
        EXPECT_GT(cumsum, 0.8f); // But close to 0.9
    }
    ...
    TEST_F(TopPTest, TopPMonotonicityPreservation)
    {
        ...
        // Among kept tokens, order should be preserved
        std::vector<std::pair<float, size_t>> kept_tokens;
        for (size_t i = 0; i < filtered.size(); ++i)
        {
            if (filtered[i] > 1e-6f)
            {
                kept_tokens.push_back({filtered[i], i});
            }
        }

        // Probabilities should be decreasing (monotonicity preservation)
        for (size_t i = 1; i < kept_tokens.size(); ++i)
        {
            EXPECT_LT(kept_tokens[i].first, kept_tokens[i - 1].first);
        }
    }
