...
void ChannelMixingLayer::initialize() {
  ...
  printf("[CM] shift_ resized successfully\n");
  fflush(stdout);
  printf("[CM] shift_.size() = %zu, shift_.capacity() = %zu\n", shift_.size(), shift_.capacity());
  fflush(stdout);
  
  printf("[CM] Starting shift fill loop\n");
  fflush(stdout);
  
  for (uint32_t i = 0; i < hidden_dim_; ++i) {
    printf("[CM]   i=%u, setting shift[%u]\n", i, i);
    fflush(stdout);
    shift_[i] = 0.01f;
    printf("[CM]   shift[%u] = 0.01f ok\n", i);
    fflush(stdout);
  }
  ...
  printf("[CM] proj_size calculated as %u\n", proj_size);
  fflush(stdout);
  key_proj_w_.resize(proj_size);
  printf("[CM] key_proj_w_ resized to %zu\n", key_proj_w_.size());
  fflush(stdout);
  printf("[CM] Filling key_proj_w\n");
  fflush(stdout);
  for (uint32_t i = 0; i < proj_size; ++i) {
    if (i % 1000 == 0) {
      printf("[CM]   key_proj_w[%u]\n", i);
      fflush(stdout);
    }
    key_proj_w_[i] = 0.01f;
  }
  ...
  for (uint32_t i = 0; i < proj_size; ++i) {
    if (i % 1000 == 0) {
      printf("[CM]   value_proj_w[%u]\n", i);
      fflush(stdout);
    }
    value_proj_w_[i] = 0.01f;
  }
