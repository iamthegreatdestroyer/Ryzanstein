...
struct SpeculativeStats {
    ...
    float get_overall_speedup() const {
        if (total_forward_passes == 0) return 1.0f;
    ...
    float get_theoretical_max_speedup() const {
        if (draft_stats.num_inferences == 0) return 1.0f;
    ...
    float get_efficiency_percent() const {
        ...
        if (theoretical < 1.0f) return 100.0f;
...
public:
    ...
    uint32_t get_K() const {
        if (draft_model_) return draft_model_->get_current_K();
        return 0;
    }
