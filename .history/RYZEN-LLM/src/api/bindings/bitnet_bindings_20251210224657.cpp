...
extern "C" {
    ...
    // Test function that creates a TernaryWeight with parameters
    __declspec(dllexport) int test_create_ternary_weight_params() {
        try {
            std::cout << "DEBUG: test_create_ternary_weight_params called" << std::endl;
            // Create a TernaryWeight with parameters (this calls resize)
            ryzen_llm::bitnet::TernaryWeight* weight = new ryzen_llm::bitnet::TernaryWeight(4, 4, 0);
            std::cout << "DEBUG: TernaryWeight with params created successfully" << std::endl;
            delete weight;
            return 1; // Success
        } catch (const std::exception& e) {
            std::cout << "DEBUG: Exception creating TernaryWeight with params: " << e.what() << std::endl;
            return -1; // Failure
        } catch (...) {
            std::cout << "DEBUG: Unknown exception creating TernaryWeight with params" << std::endl;
            return -2; // Failure
        }
    }
