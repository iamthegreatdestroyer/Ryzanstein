...
AttentionParams create_test_attention(uint32_t hidden_dim, uint32_t num_heads) {
    ...
    for (auto& w : attn.W_q) w = dist(gen);
    for (auto& w : attn.W_k) w = dist(gen);
    for (auto& w : attn.W_v) w = dist(gen);
    for (auto& w : attn.W_o) w = dist(gen);
...
FFNParams create_test_ffn(uint32_t hidden_dim, uint32_t ffn_dim) {
    ...
    for (auto& w : ffn.W_up) w = dist(gen);
    for (auto& w : ffn.W_down) w = dist(gen);
...
ModelWeights create_test_weights(const ModelConfig& config) {
    ...
    std::uniform_int_distribution<int8_t> ternary_dist(-1, 1);
...
void test_single_layer() {
    ...
    for (auto x : output) output_mean += x;
...
void test_full_model() {
    ...
    for (auto l : logits) logits_sum += l;
