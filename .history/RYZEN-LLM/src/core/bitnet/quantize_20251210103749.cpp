...
void dequantize_activations(
    ...
    for (size_t i = 0; i < size; ++i) {
        output[i] = (static_cast<float>(quantized.values[i]) - quantized.zero_point) 
                   * quantized.scale;
    }
}
...
std::vector<uint8_t> pack_ternary_weights(const TernaryWeight& ternary_weight) {
    ...
    for (size_t i = 0; i < total_size; ++i) {
        ...
        switch (ternary_weight.values[i]) {
            ...
            default: code = 0b01; break;  // Invalid, treat as 0
        }
...
TernaryWeight unpack_ternary_weights(
    ...
    for (size_t i = 0; i < total_size; ++i) {
        ...
        // Map: 00 -> -1, 01 -> 0, 10 -> +1
        switch (code) {
            case 0b00: result.values[i] = -1; break;
            case 0b01: result.values[i] =  0; break;
            case 0b10: result.values[i] =  1; break;
            default:   result.values[i] =  0; break;
        }
    }
