...
namespace ryzen_llm
{
    namespace avx512
    {
              ...
              has_avx512_vbmi(false)
        {
            ...
            // Check for AVX-512 Foundation (leaf 7, subleaf 0)
                    ...
                    for (uint32_t k_tile = 0; k_tile < K; k_tile += TILE_K)
                    {
                        ...
                        for (uint32_t m = m_tile; m < m_end; ++m)
                        {
                            for (uint32_t n = n_tile; n < n_end; ++n)
                            {
                                ...
                                {
                                    // Load 16 ternary weights
                                    __m128i weights_i8 = _mm_loadu_si128(
                                        reinterpret_cast<const __m128i *>(&weights.values[m * K + k]));

                                    // Load 16 INT8 activations
                                    __m128i activations_i8 = _mm_loadu_si128(
                                        reinterpret_cast<const __m128i *>(&activations.values[k * N + n]));
                                    ...
                                    // Load weight scales (16 scales per vector)
                                    __m512 w_scales = _mm512_loadu_ps(&weights.scales[m * K + k]);

                                    // Apply weight scaling
                                    __m512 w_scaled = _mm512_mul_ps(w_fp32, w_scales);

                                    // Multiply-accumulate: acc += w_scaled * act_scaled
                                    acc = _mm512_fmadd_ps(w_scaled, act_scaled, acc);
                                }
                                ...
                                for (; k < k_end; ++k)
                                {
                                    ...
                                    sum += scaled_weight * dequantized_x;
                                }
