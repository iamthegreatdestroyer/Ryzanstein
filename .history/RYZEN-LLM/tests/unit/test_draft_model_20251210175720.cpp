...
TEST_F(DraftModelTest, ConstructorThrowsOnZeroVocabSize)
{
    ...
    config.vocab_size = 0;
    EXPECT_THROW(DraftModel(config), std::invalid_argument);
}
...
TEST_F(DraftModelTest, ConstructorThrowsOnInvalidAcceptanceRateTarget)
{
    ...
    config.acceptance_rate_target = 1.1f;
    EXPECT_THROW(DraftModel(config), std::invalid_argument);
}
...
{
    DraftModel model(default_config);
    std::vector<int> prefix = {100, 200, default_config.vocab_size}; // Last token invalid
...
TEST_F(DraftModelEdgeCaseTest, MinimalValidConfig)
{
    DraftModelConfig minimal_config{
        ...
        .min_K = 1,
        .max_K = 1,
        .K_adjust_frequency = 1,
        .temperature = 1.0f,
        .top_k = 0,
        .top_p = 1.0f,
        .acceptance_rate_target = 0.5f,
        .enable_statistics = false};

    EXPECT_NO_THROW(DraftModel model(minimal_config));
}
...
TEST_F(DraftModelEdgeCaseTest, MaxKEqualsMinK)
{
    ...
    EXPECT_NO_THROW(DraftModel model(config));
    DraftModel model(config);
    EXPECT_EQ(model.get_current_K(), 4);
}
