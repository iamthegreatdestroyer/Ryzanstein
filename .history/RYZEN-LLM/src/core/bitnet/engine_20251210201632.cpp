    ...
    namespace bitnet
    {
        ...
        bool BitNetEngine::parse_safetensors_header(const std::string &header_json, const char *data_start, size_t data_size)
        {
            ...
            while (pos < header_json.size())
            {
                ...
                if (shape_start == std::string::npos) break;

                size_t shape_end = header_json.find("]", shape_start);
                if (shape_end == std::string::npos) break;
                ...
                if (offsets_start == std::string::npos) break;

                size_t offsets_end = header_json.find("]", offsets_start);
                if (offsets_end == std::string::npos) break;
            ...
            {
                if (tensor.data_offsets.size() != 2) continue; // Invalid offsets

                uint64_t data_offset = tensor.data_offsets[0];
                uint64_t data_size = tensor.data_offsets[1] - tensor.data_offsets[0];

                if (data_offset + data_size > data_size) continue; // Out of bounds
                ...
                else if (tensor.name.find("layers.") == 0 && tensor.name.find(".attention.") != std::string::npos)
                {
                    ...
                    if (layer_idx >= config_.num_layers) continue;
