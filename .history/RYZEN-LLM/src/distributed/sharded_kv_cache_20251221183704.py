...
class ShardedKVCache(DistributedKVCache):
    ...
    def __init__(
        ...
        """
        super().__init__(max_seq_len, hidden_size, num_layers, num_heads)
        ...
        logger.info(f"Initialized ShardedKVCache: rank {rank}/{world_size}, "
    ...
    def update(
        ...
        if self.use_compression:
            local_key, local_value = self._compress_kv(local_key, local_value)
    ...
    def get(
        ...
        # Get local shard
        local_key = self.cache_k[layer_idx, :, :, start_pos:start_pos+seq_len, :]
        local_value = self.cache_v[layer_idx, :, :, start_pos:start_pos+seq_len, :]

        # Transpose back to (batch, seq_len, local_heads, head_dim)
        local_key = local_key.transpose(1, 2)
        local_value = local_value.transpose(1, 2)

        # Decompress if needed
        if self.use_compression:
            local_key, local_value = self._decompress_kv(local_key, local_value)
