...
PYBIND11_MODULE(ryzen_llm_bindings, m) {
    ...
    m.def("test_function", []() {
        return std::string("Ryzen-LLM BitNet extension loaded successfully!");
    }, "Test function to verify extension loading");
    ...
    m.def("quantize_activations_int8", [](const std::vector<float>& activations, size_t size) {
        ...
        return ryzen_llm::bitnet::quantize_activations_int8(activations.data(), size, config);
    ...
    m.def("quantize_weights_ternary", [](const std::vector<float>& weights, uint32_t rows, uint32_t cols) {
        ryzen_llm::bitnet::QuantConfig config; // Use default config
        return ryzen_llm::bitnet::quantize_weights_ternary(weights.data(), rows, cols, config);
    }, pybind11::arg("weights"), pybind11::arg("rows"), pybind11::arg("cols"),
