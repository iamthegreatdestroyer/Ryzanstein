...
namespace ryzen_llm
{
    namespace bitnet
    {
            ...
            const QuantConfig &config)
        {
            ...
            std::cout << "DEBUG: per_group_scaling=" << config.per_group_scaling << std::endl;
            const size_t total_size = static_cast<size_t>(rows) * cols;
            std::cout << "DEBUG: total_size=" << total_size << std::endl;
            ...
            else
            {
                ...
                // Try manual division instead of /= operator
                float temp = mean_abs;
                mean_abs = temp / static_cast<float>(total_size);
