...
void ChannelMixingLayer::matrix_multiply_(
  ...
  printf("[CM.matrix_mult] matrix_multiply_() called with m=%u, n=%u\n", m, n);
  fflush(stdout);
  
  // Clear output first
  std::fill(y.begin(), y.end(), 0.0f);
  
  // Manual matrix multiplication using simple additions
  // y[i] += A[i*n + j] * x[j] for all j
  for (uint32_t i = 0; i < m; ++i) {
    float row_sum = 0.0f;
    
    // Manually unroll inner loop to avoid the mysterious for loop bug
    uint32_t j = 0;
    while (j < n) {
      uint32_t idx = i * n + j;
      row_sum = row_sum + (A[idx] * x[j]);
      j++;
    }
  ...
  printf("[CM.matrix_mult] Matrix multiplication done\n");
  fflush(stdout);
