...
def check_build_tools():
    ...
    for tool, name in tools.items():
        try:
            if tool.endswith('.exe'):
                result = subprocess.run(f'where.exe {tool}', shell=True, capture_output=True)
            else:
    ...
    if missing:
        ...
        print("\n📦 Please install:")
        print("- Visual Studio 2022 with C++ development tools")
        print("- CMake (latest version)")
...
def configure_cmake(build_dir: Path, cpp_build_dir: Path):
    ...
    cmake_cmd = f'cmake -S "../../src" -B "." -DCMAKE_BUILD_TYPE=Release'

    # Add Python-specific configuration
    python_executable = sys.executable
    python_include = sys.base_prefix + "/include"
    python_library = sys.base_prefix + "/libs/python313.lib"  # Adjust for your Python version
